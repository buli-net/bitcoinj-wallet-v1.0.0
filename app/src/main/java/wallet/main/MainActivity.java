 ...
 * Owns the primary wallet screen, toolbar actions, system-theme presentation, QR entry, backup export, and view binding.
...
public class MainActivity extends AppCompatActivity
    ...
    @Override
    @UiThread
    public void displayWalletPath(String path) {
    }

    @Override
    @UiThread
    public void displayTransactionHistory(java.util.List<String> historyItems) {
        ...
        String sentLabel = getString(R.string.transaction_sent);
        String receivedLabel = getString(R.string.transaction_received);

        for (int i = 0; i < count; i++) {
            String item = historyItems.get(i);
            String[] lines = item == null
                    ? new String[0]
                    : item.trim().split("\\r?\\n");
            ...
            if (lines.length > 0) {
                ...
                android.text.SpannableString styled =
                        new android.text.SpannableString(firstLine);

                int typeStart = -1;
                String typeLabel = null;
                if (firstLine.contains(sentLabel)) {
                    typeLabel = sentLabel;
                    typeStart = firstLine.indexOf(sentLabel);
                } else if (firstLine.contains(receivedLabel)) {
                    typeLabel = receivedLabel;
                    typeStart = firstLine.indexOf(receivedLabel);
                }

                if (typeStart >= 0 && typeLabel != null) {
                    styled.setSpan(
                            new android.text.style.StyleSpan(android.graphics.Typeface.BOLD),
                            typeStart,
                            typeStart + typeLabel.length(),
                            android.text.Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
                    );

                    int amountStart = typeStart + typeLabel.length();
                    while (amountStart < firstLine.length()
                            && Character.isWhitespace(firstLine.charAt(amountStart))) {
                        amountStart++;
                    }

                    if (amountStart < firstLine.length()) {
                        styled.setSpan(
                                new android.text.style.StyleSpan(android.graphics.Typeface.BOLD),
                                amountStart,
                                firstLine.length(),
                ...
                summary.setText(styled);
                summary.setTextSize(13);
                summary.setTextColor(ThemeTextColors.primary(this));
                transaction.addView(summary);
            }

            for (int lineIndex = 1; lineIndex < lines.length; lineIndex++) {
                String lineText = lines[lineIndex].trim();
                if (lineText.isEmpty()) continue;

                TextView details = new TextView(this);
                details.setText(lineText);
                details.setTextSize(12);
                details.setTextColor(ThemeTextColors.primary(this));
                details.setPadding(0, 4, 0, 0);
                transaction.addView(details);
            }
    ...
    @Override
    @UiThread
    public void displayTransactionHistoryPages(int currentPage, int pageCount) {
    }
    @Override
    @UiThread
    ...
    public void displaySendDetails(
            String feeRate, String fee, String total, String remaining) {
    }
    @Override
    ...
    private void copyWalletBackup(android.net.Uri destination) {
        ...
        if (!source.exists()) {
            showToastMessage(getString(R.string.wallet_backup_source_missing));
            return;
        }
        new Thread(() -> {
            try (java.io.InputStream in = new java.io.FileInputStream(source);
                ...
                if (out == null) throw new java.io.IOException(getString(R.string.copy_destination_failed));
                byte[] buffer = new byte[8192];
                int n;
                while ((n = in.read(buffer)) != -1) out.write(buffer, 0, n);
                out.flush();
                showToastMessage(getString(R.string.backup_success));
            } catch (Exception e) {
                showToastMessage(getString(R.string.backup_failed, e.getMessage()));
            }
