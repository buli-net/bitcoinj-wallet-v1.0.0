...
public class SendActivity extends AppCompatActivity implements SendPresenter.View {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        if (getSupportActionBar() != null) {
            getSupportActionBar().setTitle("Send Bitcoin");
        ...
        fee.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(SeekBar b, int p, boolean user) { updateFeeLabel(); }
            public void onStartTrackingTouch(SeekBar b) { }
            public void onStopTrackingTouch(SeekBar b) { }
        });
        updateFeeLabel();
        amount.setOnFocusChangeListener((v, has) -> { if (has && "0.00".equals(amount.getText().toString())) amount.selectAll(); });
        scan.setOnClickListener(v -> new IntentIntegrator(this).setPrompt("Scan recipient address").initiateScan());
        ...
        new Thread(() -> {
            try {
                ...
                runOnUiThread(() -> {
                    ...
                    try { Bitmap qr = QRCode.from(address).bitmap(); myQr.setImageBitmap(qr); } catch (Exception ignored) { }
                });
            } catch (Exception e) { runOnUiThread(() -> showMessage("Không đọc được wallet: " + e.getMessage())); }
    ...
    private void fillMax() {
        ...
        new Thread(() -> {
            ...
            } catch (Exception e) { showMessage("Không đọc được số dư"); }
        }).start();
    }

    private void updateFeeLabel() {
        int satVb = fee.getProgress() + 1;
        feeLabel.setText("Fee rate: " + satVb + " sat/vB (" + (satVb * 1000) + " sat/vkB)");
        estimatedFee.setText("Transaction fee (estimated)\nFee will be calculated from the real transaction size before broadcast.");
    }

    @Override public String recipient() { return recipient.getText().toString(); }
    @Override public String amount() { return amount.getText().toString(); }
    @Override public int feeSatVb() { return fee.getProgress() + 1; }
    @Override public void showMessage(String message) { runOnUiThread(() -> Toast.makeText(this, message, Toast.LENGTH_LONG).show()); }
    @Override public void showSending(boolean sending) { runOnUiThread(() -> send.setEnabled(!sending)); }
    @Override public void clearForm() { runOnUiThread(() -> { amount.setText("0.00"); recipient.setText(""); loadWalletDisplay(); }); }

    @Override public void showConfirmation(String details, boolean feeWarning, Runnable confirm) {
        runOnUiThread(() -> new AlertDialog.Builder(this)
                .setTitle(feeWarning ? "Warning: actual fee is higher" : "Confirm transaction")
                .setMessage(details)
                .setNegativeButton("CANCEL", null)
                .setPositiveButton(feeWarning ? "SEND ANYWAY" : "SEND", (d, w) -> confirm.run())
                .show());
    }
