...
public class SendActivity extends AppCompatActivity implements SendPresenter.View {
    ...
    private void styleConfirmationButtons(AlertDialog dialog) {
        ...
        styleDialogButton(negative, R.drawable.bg_dialog_button_outline,
                resolveThemeColor(android.R.attr.colorAccent), height);
        styleDialogButton(positive, R.drawable.bg_dialog_button_primary,
                resolveThemeColor(android.R.attr.textColorPrimaryInverse), height);

        ViewGroup.LayoutParams negativeParams = negative.getLayoutParams();
        if (negativeParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) negativeParams;
            params.width = 0;
            params.height = height;
            params.weight = 1f;
            params.setMargins(side, 0, gap / 2, side);
            negative.setLayoutParams(params);
        }

        ViewGroup.LayoutParams positiveParams = positive.getLayoutParams();
        if (positiveParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) positiveParams;
            params.width = 0;
            params.height = height;
            params.weight = 1f;
            params.setMargins(gap / 2, 0, side, side);
