...
public class SendActivity extends AppCompatActivity implements SendPresenter.View {
    ...
    private View buildConfirmationContent(String details) {
        ...
        for (int index = 0; index < sections.length; index++) {
            ...
            text.setPadding(0, dpToPx(8), 0, dpToPx(8));
    ...
    private CharSequence buildConfirmationSection(String section) {
        ...
        for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            boolean fullLineImportant =
                    startsWithResource(
                            line,
                            R.string.details_full_balance)
                            || startsWithResource(
                                    line,
                                    R.string.details_fee_note);

            if (fullLineImportant) {
                result.setSpan(
            ...
            if (colon > 0) {
                ...
                result.setSpan(
                    ...
                    getString(R.string.details_fee_details).trim())) {
                result.setSpan(
                        new android.text.style.StyleSpan(Typeface.BOLD),
                        start,
                        result.length(),
    ...
    private boolean startsWithResource(
            ...
            int resourceId,
            Object... formatArgs) {
        String prefix = getString(resourceId, formatArgs);
