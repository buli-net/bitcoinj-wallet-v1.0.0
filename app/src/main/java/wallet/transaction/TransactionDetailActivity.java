...
public final class TransactionDetailActivity extends AppCompatActivity {
    ...
    private void renderEntries(LinearLayout container, List<TxEntry> entries, int totalString) {
        ...
        for (TxEntry entry : entries) {
            ...
            // Force the address TextView to use the full row width. With WRAP_CONTENT,
            // a long address can measure wider than the card and get clipped instead of
            // receiving a real one-line middle ellipsis. The underlying text remains
            // unchanged, so copy/select still returns the complete address.
