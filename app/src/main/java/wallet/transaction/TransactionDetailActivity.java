...
public final class TransactionDetailActivity extends AppCompatActivity {
    ...
    private TransactionOutput findConnectedOutput(
        ...
        try {
            Method method = input.getClass().getMethod("getConnectedOutput");
            Object connected = method.invoke(input);
            if (connected instanceof TransactionOutput) {
                return (TransactionOutput) connected;
            }
        } catch (Exception ignored) {
        }
